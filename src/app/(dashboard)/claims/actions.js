...
export async function getWarrantyClaims(options = {}) {
  try {
    const response = await fetchWithAuth(`${CLAIMS_ENDPOINT}?${buildListQuery(options)}`, {
      cache: 'no-store',
    });

    return normalizeListResponse(response, 'Failed to fetch warranty claims');
  } catch (error) {
...
export async function getWarrantyClaimById(id) {
  if (!id) throw new Error('Warranty claim ID is required');

  const response = await fetchWithAuth(`${CLAIMS_ENDPOINT}/${id}`, {
    cache: 'no-store',
  });

  if (response?.code !== 200) {
    throw new Error(response?.message || 'Failed to fetch warranty claim');
  }
...
export async function createWarrantyClaim(payload) {
  try {
    ...
    return normalizeMutationResponse(response, 'Failed to create warranty claim');
  } catch (error) {
...
export async function updateWarrantyClaim(id, payload) {
  try {
    ...
    return normalizeMutationResponse(response, 'Failed to update warranty claim');
  } catch (error) {
    return { success: false, message: error.message };
  }
}
