...
const average = values => {
  const list = Array.isArray(values)
    ? values.map(toFiniteNumber).filter(v => Number.isFinite(v))
    : [];
...
const sum = values =>
  (Array.isArray(values) ? values : []).reduce((acc, value) => acc + toFiniteNumber(value), 0);
...
const stdDev = values => {
  ...
  const variance = average(list.map(v => (v - mean) ** 2));
...
const conciseSentence = (value, maxLen = 90) => {
  ...
  if (first.length <= maxLen) return `${first.replace(/\.*$/, '')}.`;
...
const buildTemporalSignals = labels => {
  ...
  return list.map(label => {
    ...
    if ([5, 6, 7].includes(monthIndex)) events.push('Heat-driven beverage and cooling-product demand');
    if (monthIndex === 11) events.push('Year-end budget resets and holiday retail volatility');
...
const buildBusinessDiagnostics = ({
  ...
  const cleanSales = salesActual.filter(v => v !== null && v !== undefined).map(toFiniteNumber);
  ...
  const highAlerts = alerts.filter(item => String(item?.severity || '').toLowerCase() === 'high')
    .length;
...
const stabilizeFutureSeries = ({
  ...
  for (let i = 0; i < horizon; i++) {
    const candidate = toFiniteNumber(candidateFuture?.[i]);
    const baseline = toFiniteNumber(baselineFuture?.[i]);
    const fallback = i > 0 ? output[i - 1] : cleanRecent[cleanRecent.length - 1] ?? baseline;
    let nextValue =
...
const stabilizeBacktestSeries = ({
  ...
  for (let i = 0; i < horizon; i++) {
    const candidate = toFiniteNumber(candidateBacktest?.[i]);
    const baseline = toFiniteNumber(baselineBacktest?.[i]);
    const actual = toFiniteNumber(referenceActual?.[i]);
    const prev = i > 0 ? output[i - 1] : baseline || actual || 0;
    const anchor = average([
      actual,
      baseline,
      prev,
      toFiniteNumber(referenceActual?.[i - 1]),
      toFiniteNumber(referenceActual?.[i - 2]),
    ]);
...
const buildMetricSeries = (baselineMetric, aiMetric, allowNegative = false) => {
  ...
  const baselineForecast = Array.isArray(baselineMetric?.forecast)
    ...
    : [];
  const firstFutureIndexRaw = actual.findIndex(value => value === null || value === undefined);
  ...
  for (let i = 0; i < backtestWindow; i++) {
    mergedForecast[backtestStart + i] =
      stabilizedBacktest[i] ??
      baselineBacktest[i] ??
      mergedForecast[backtestStart + i] ??
      0;
  }
  for (let i = 0; i < horizon; i++) {
    mergedForecast[firstFutureIndex + i] =
      stabilizedFuture[i] ?? baselineFuture[i] ?? mergedForecast[firstFutureIndex + i] ?? 0;
  }
...
const buildFallbackInsightCards = ({ labels, mergedSeries, inventoryAlerts, topProducts }) => {
  ...
  const salesActual = Array.isArray(sales.actual)
    ? sales.actual.filter(v => v !== null && v !== undefined).map(toFiniteNumber)
    : [];
  const latestSalesActual = salesActual[salesActual.length - 1] || 0;
  const salesDelta =
    latestSalesActual > 0
      ? ((toFiniteNumber(sales.forecastNext) - latestSalesActual) / Math.abs(latestSalesActual)) * 100
      : 0;
  ...
  const cards = [
    ...
    },
    {
      ...
      title: 'Purchase plan',
      text:
        toFiniteNumber(expenses.forecastNext) > toFiniteNumber(expenses.actual?.[expenses.firstFutureIndex - 1] || 0)
