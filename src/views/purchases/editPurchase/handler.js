...
import { purchaseSchema } from '../addPurchase/PurchaseSchema';
...
function useFormHandler({ purchaseData }) {
  const today = new Date().toISOString().split('T')[0];
  const purchaseNumber = purchaseData?.purchaseId || '';
  const paymentMethod = purchaseData?.payment_method || purchaseData?.paymentMode || '';

  const defaultValues = {
    id: purchaseData?._id || '',
    purchaseNumber,
    purchaseId: purchaseNumber,
    purchaseDate: purchaseData?.purchaseDate ? new Date(purchaseData.purchaseDate).toISOString().split('T')[0] : today,
    dueDate: purchaseData?.dueDate ? new Date(purchaseData.dueDate).toISOString().split('T')[0] : today,
    ...
    employee: purchaseData?.employee || '',
    employeeImage: purchaseData?.employeeImage || '',
    notes: purchaseData?.notes || '',
    termsAndCondition: purchaseData?.termsAndCondition || '',
    items: mapPurchaseItems(purchaseData?.items),
  };
...
function useSubmissionHandler({ trigger, closeSnackbar, enqueueSnackbar, getValues, onSave }) {
  ...
  const handleFormSubmit = async data => {
    try {
      ...
      const result = await onSave(purchasePayload, employeeURL);
      ...
      enqueueSnackbar?.(`Failed to update purchase: ${error.message}`, {
        variant: 'error',
...
export default function useEditPurchaseHandlers({
  purchaseData,
  productData,
  initialBanks,
  employees,
