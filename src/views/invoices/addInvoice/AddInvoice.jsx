...
const AddInvoice = ({ customersData, productData, taxRates, initialBanks, signatures, onSave, enqueueSnackbar, closeSnackbar, invoiceNumber }) => {
  ...
  const cashierStorageKey = 'addInvoice.cashierSignatureId';
  const selectedCashierSignatureId = watch('signatureId');
  ...
  // const checkoutSummarySpacer = '150px';
  ...
  // Persist cashier selection across refresh
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const stored = window.localStorage.getItem(cashierStorageKey);
    if (!stored) return;

    const options = Array.isArray(signOptions) ? signOptions : [];
    const shouldValidate = options.length > 0;
    if (shouldValidate && !options.some((sig) => sig?._id === stored)) {
      window.localStorage.removeItem(cashierStorageKey);
      return;
    }
  ...
  }, [cashierStorageKey, signOptions, getValues, setValue]);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(cashierStorageKey, selectedCashierSignatureId);
  }, [cashierStorageKey, selectedCashierSignatureId]);
  ...
  const handleBarcodeScan = useCallback((barcodeValue) => {
    ...
    setSnackbar({ open: true, message: 'Product added', severity: 'success' });
  }, [productData, getValues, handleAddEmptyRow, handleUpdateItemProduct, setSnackbar]);
  ...
  const handleUseWalkInCustomer = () => {
    setIsWalkIn(true);
    setValue('customerId', walkInCustomerId);
    setSnackbar({ open: true, message: 'Walk-in enabled', severity: 'success' });
  };
  ...
  const loadHeldSale = (sale) => {
    ...
    if (sale.isWalkIn) {
      setValue('customerId', sale.customerId && sale.customerId !== 'walk-in' ? sale.customerId : walkInCustomerId);
    } else {
        ...
        <DialogContent sx={{ py: 4 }}>
          <Box className="flex flex-col gap-2">
            ...
            <Typography variant="body2" color="text.secondary">
              Invoice: {posReceiptData?.invoiceNumber || invoiceNumber}
            </Typography>
            <Typography variant="body2" color="text.secondary">
