...
const AddPurchaseReturn = ({
  ...
  // Products clone data for the columns
  const productsCloneData = [...products];
  ...
  // Sync form values with handlers
  useEffect(() => {
    if (handlers.formData.debit_note_id) {
      setValue('debit_note_id', handlers.formData.debit_note_id);
    }
    if (handlers.formData.purchaseOrderDate) {
      setValue('purchaseOrderDate', handlers.formData.purchaseOrderDate);
    }
    if (handlers.formData.dueDate) {
      setValue('dueDate', handlers.formData.dueDate);
    }
  }, [handlers.formData, setValue]);
  ...
  return (
    <form onSubmit={handleSubmit((data) => {
      // Merge form data with handlers form data for submission
      ...
      handlers.handleSubmit(submissionData);
    })}>
      <div>
        {/* Header */}
                ...
                <CardContent className="p-6">
                  <div className="space-y-3">
                    <Button
                      ...
                      fullWidth
                      size="large"
                      disabled={handlers.isSubmitting || fields.length === 0}
                      startIcon={
