...
export const DebitNoteSchema = yup.object().shape({
  ...
  dueDate: yup
    ...
    .test(
      'is-after-debit-note',
      'Due date must be on or after debit note date',
      function(value) {
        const debitNoteDate = this.parent.purchaseOrderDate;
        if (!debitNoteDate || !value) return true;

        const debitNote = dayjs(debitNoteDate).startOf('day');
        const due = dayjs(value).startOf('day');

        return due.isSameOrAfter(debitNote);
      }
    ),
