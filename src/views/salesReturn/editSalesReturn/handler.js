...
export default function useSalesReturnHandlers({
  ...
  useEffect(() => {
    if (productData && salesReturnData?.items) {
      const usedProductIds = salesReturnData.items.map((item) => item.productId?._id || item.productId);
      const availableProducts = productData.filter((product) => !usedProductIds.includes(product._id));
  ...
  const handleError = (formErrors) => {
    ...
    } else if (formErrors.items && formErrors.items.length > 0) {
      firstError = 'Please check product details';
    } else if (formErrors.bank) {
