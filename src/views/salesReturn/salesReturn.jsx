...
const SalesReturn = ({ mode = 'add', title = 'Add Sales Return', documentNumber = '', recordId = '', customersData, taxRates, handlers, columnsFactory }) => {
  ...
  const emptyContent = (
    <Box className="flex flex-col items-center justify-center py-6 gap-2 text-center">
      ...
      <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 300 }}>Click the Add Item button to add items to your sales return.</Typography>
      ...
      <Grid size={{ xs: 12 }}>
        <div className="flex items-center gap-2">
          <div className="bg-primary/12 text-primary bg-primaryLight w-12 h-12 rounded-full flex items-center justify-center">
            <Icon icon="mdi:invoice-export-outline" fontSize={26} />
          </div>
          ...
          <CardContent className="py-4">
            <Grid container spacing={3} alignItems="center">
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField label="Sales Return No" value={documentNumber || getValues('purchaseOrderNumber') || getValues('debitNoteNumber') || getValues('salesReturnNumber') || ''} variant="outlined" fullWidth size="medium" InputProps={{ readOnly: true }} />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  fullWidth
              ...
              <Grid size={{ xs: 12, md: 3 }}>
                <Controller name="salesReturnDate" control={control} render={({ field }) => (
                  <TextField {...field} label="Sales Return Date" type="date" variant="outlined" fullWidth size="medium" error={!!errors.salesReturnDate} inputProps={{ max: new Date().toISOString().split('T')[0] }} InputLabelProps={{ shrink: true }} />
                )} />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
      ...
      <Drawer anchor="right" open={drawerOpen} onClose={() => setDrawerOpen(false)} PaperProps={{ sx: { width: { xs: '100%', sm: 400 }, px: 6, py: 5 } }}>
        <Box className="flex items-center justify-between">
          <Box className="flex items-center gap-2">
            ...
            <Typography variant="h6" sx={{ fontWeight: 600 }}>Sales Return Options</Typography>
          </Box>
