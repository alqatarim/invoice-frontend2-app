                   ...
                   <Grid container spacing={5} >
                      ...
                      <Grid item xs={6} sx={{ textAlign: 'left' }}>
                        <Typography variant='h6' fontWeight={500}>
                          #{salesReturnData?.salesReturnNumber}
                        </Typography>
                        <Typography variant='h6'  fontWeight={500}>
                          {dayjs(salesReturnData?.salesReturnDate).format('DD MMM YYYY')}
                        </Typography>
                        <Typography variant='h6'  fontWeight={500}>
                          {dayjs(salesReturnData?.dueDate).format('DD MMM YYYY')}
                        </Typography>
                      </Grid>
                   </Grid>
                </Grid>
            ...
            <Grid item>
              ...
              padding={4} mb={4} className='flex justify-between'>
                <Grid item xs={'auto'} sx={{ textAlign: 'left' }}>
                  ...
                  <Typography className='text-[14px]'>{salesReturnData?.customerId?.name}</Typography>
                  <Typography className='text-[14px]'>{salesReturnData?.customerId?.billingAddress?.addressLine1}</Typography>
                  <Typography className='text-[14px]'>{salesReturnData?.customerId?.billingAddress?.city}</Typography>
                  <Typography className='text-[14px]'>{salesReturnData?.customerId?.email}</Typography>
                  <Typography className='text-[14px]'>{salesReturnData?.customerId?.phone}</Typography>
                </Grid>
                       ...
                       <Grid item xs={12}>
                          <Grid container className='flex flex-row justify-center gap-3'>
                              ...
                              <Grid item xs='auto' sx={{ textAlign: 'left' }} className='flex flex-col gap-0.5'>
                                  <Typography className='text-[14px]' fontWeight={500}>{salesReturnData?.bank?.name}</Typography>
                                  <Typography className='text-[14px]' fontWeight={500}>{salesReturnData?.bank?.bankName}</Typography>
                                  <Typography className='text-[14px]' fontWeight={500}>{salesReturnData?.bank?.accountNumber}</Typography>
                                  <Typography className='text-[14px]' fontWeight={500}>{salesReturnData?.bank?.branch}</Typography>
                              </Grid>
                          </Grid>
                      </Grid>
                  </Grid>
            ...
            <Grid item padding={5}>
              <Grid container spacing={4}  className='justify-between'>
                ...
                <Grid item xs={7}>
                  ...
                  <Typography variant="body2">{salesReturnData?.termsAndCondition}</Typography>
                  <Typography variant="subtitle2" gutterBottom>
                    Notes:
                  </Typography>
                     ...
                     <Grid item xs={12} className='flex flex-col gap-0' >
                      <Grid container  spacing={1} alignItems="center" justifyContent='space-between'>
                        <Grid item xs={6} className='flex flex-col gap-1' sx={{ textAlign: 'right'}}>
                          ...
                          <Typography className='text-[14px]'  color="text.secondary">VAT ({salesReturnData?.items?.[0]?.form_updated_tax}%):</Typography>
                        </Grid>

                        <Grid item xs={'auto'} className='flex flex-col gap-1' sx={{ textAlign: 'right'}}>
                          <Typography className='text-[14px]'  >{formatCurrency(salesReturnData?.taxableAmount)}</Typography>
                          <Typography className='text-[14px]'  >{`(${formatCurrency(salesReturnData?.totalDiscount)})`}</Typography>
                          <Typography className='text-[14px]'  >{formatCurrency(salesReturnData?.vat)}</Typography>
                        </Grid>
                      </Grid>
                    </Grid>
                    ...
                    <Grid item xs={12} className='flex flex-col gap-1.5' sx={{ textAlign: 'right'}}>
                      <Grid container  spacing={1} justifyContent='space-between'>
                        ...
                        <Grid item xs={'auto'} className='flex flex-col gap-1.5' sx={{ textAlign: 'right'}}>
                          <Typography className='text-[16px]' fontWeight={600} >{formatCurrency(salesReturnData?.TotalAmount)}</Typography>
                        </Grid>
                      </Grid>
                    </Grid>
                   </Grid>
