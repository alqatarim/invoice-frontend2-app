...
import { getCategoryDetails } from '@/app/(dashboard)/products/actions';
import { useEditCategoryHandlers } from '@/handlers/products/category/editCategory';
    ...
    <Dialog fullWidth open={open} onClose={handleClose} maxWidth='sm' scroll='body'>
      ...
      <DialogContent className='overflow-visible pbs-0 pbe-6 pli-12 sm:pli-12'>
        ...
        {loading ? (
          ...
          <Box className="flex justify-center items-center h-40">
            <Typography>Failed to load category data</Typography>
          </Box>
        )}
      </DialogContent>
      ...
      <DialogActions className='flex justify-center pbs-0 pbe-10 pli-10 sm:pbe-16 sm:pli-16'>
        ...
        <Button 
          ...
          disabled={isSubmitting || !categoryData}
        >
          {isSubmitting ? <CircularProgress size={20} /> : 'Update Category'}
        </Button>
      </DialogActions>
    </Dialog>
  );
