...
const EditProductDialog = ({ open, productId, onClose, onSave, variant = 'dialog' }) => {
  ...
  const applyMetaToState = (meta) => {
    ...
    setPromotions(
      Array.isArray(meta?.promotions)
        ? meta.promotions.map((promo) => ({
          id: promo.id || createLocalId(),
          name: promo.name || '',
          discountType: promo.discountType || 'Percentage',
    ...
    setSerialTracking({
      enabled: Boolean(
        meta?.serialTracking?.enabled || serialNumbers.length > 0 || imeiNumbers.length > 0
      ),
      serialNumbers,
      imeiNumbers,
    });
  ...
  const parsedDescription = useMemo(
    () => parseProductDescription(productData?.productDescription || ''),
    [productData]
  );
  const cleanedProductData = productData
    ? { ...productData, productDescription: parsedDescription.description || '' }
    : null;
  ...
  const {
    ...
    onSave: async (data, preparedImage) => {
      ...
      const result = await onSave(productId, { ...data, productDescription: combinedDescription }, preparedImage);
      if (result.success) {
        resetAdvancedFields();
        onClose();
      }
  ...
  useEffect(() => {
    if (open && productData) {
      applyMetaToState(parsedDescription.meta || {});
    } else if (!open) {
      resetAdvancedFields();
    }
  }, [open, productData, parsedDescription]);
                        ...
                        <div className='flex items-center gap-3'>
                          ...
                          <div>
                            ...
                            <Typography variant='body2' color='text.secondary'>
                              Update the core product information
                            </Typography>
                          </div>
                        </div>
                      }
                      ...
                      <Grid container spacing={3}>
                                  ...
                                  {variants.map((variant) => (
                                    ...
                                    >
                                      ...
                                      <Box sx={{ display: 'flex', gap: 1 }}>
                                        <Button size="small" onClick={() => handleEditVariant(variant)} disabled={isSubmitting || loading}>
                                          Edit
                                        </Button>
                                        <Button
                                          size="small"
                        ...
                        <Grid size={{ xs: 12 }}>
                          <Grid container spacing={2}>
                            ...
                            <Grid size={{ xs: 12 }}>
                              <Box sx={{ display: 'flex', gap: 2 }}>
                                <Button variant="outlined" onClick={handleSavePriceTier} disabled={isSubmitting || loading}>
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                  {priceTiers.map((tier) => (
                                    ...
                                    >
                                      ...
                                      <Box sx={{ display: 'flex', gap: 1 }}>
                                        <Button size="small" onClick={() => handleEditPriceTier(tier)} disabled={isSubmitting || loading}>
                                          Edit
                                        </Button>
                                        <Button
                                          size="small"
                        ...
                        <Grid size={{ xs: 12 }}>
                          <Grid container spacing={2}>
                            ...
                            <Grid size={{ xs: 12 }}>
                              <Box sx={{ display: 'flex', gap: 2 }}>
                                <Button variant="outlined" onClick={handleSavePromotion} disabled={isSubmitting || loading}>
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                  {promotions.map((promo) => (
                                    ...
                                    >
                                      ...
                                      <Box sx={{ display: 'flex', gap: 1 }}>
                                        <Button size="small" onClick={() => handleEditPromotion(promo)} disabled={isSubmitting || loading}>
                                          Edit
                                        </Button>
                                        <Button
                                          size="small"
