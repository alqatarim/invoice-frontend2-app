...
const ProfileTab = ({ data, onUpdate, updating, error, enqueueSnackbar }) => {
  ...
  const renderPersonalInfo = () => {
    ...
    return personalData.map((item, index) => (
      <Box key={index} className='flex items-center gap-2' sx={{ minWidth: 0 }}>
        ...
        <Box
          ...
          sx={{
            minWidth: 0,
            flex: 1,
            flexWrap: item.property === 'email' ? 'nowrap' : 'wrap',
          }}
        >
          <Typography fontWeight={500} sx={{ flexShrink: 0 }}>
            {`${item.property.charAt(0).toUpperCase() + item.property.slice(1)}:`}
          </Typography>
          <Typography
            noWrap={item.property === 'email'}
            sx={
              item.property === 'email'
                ? {
                  minWidth: 0,
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid size={{ xs: 12, md: 7.5, lg: 8 }} component="form" onSubmit={handleSubmit(onSubmit)}>
