...
import { getQuotationFormColumns } from '@/views/quotations/quotationColumns';
...
const Quotation = ({
  ...
  const isEdit = title === 'Edit Quotation';
  ...
  const columns = getQuotationFormColumns({
    theme,
    control,
    errors,
    fields,
  ...
  return (
    <Grid container rowSpacing={4} columnSpacing={3}>
      <Grid size={{ xs: 12 }}>
        <div className="flex items-center gap-2">
          <div className="bg-primary/12 text-primary bg-primaryLight w-12 h-12 rounded-full flex items-center justify-center">
            <Icon icon="tabler:file-description" fontSize={26} />
          </div>
          ...
          <CardContent className="py-4">
            <Grid container spacing={3} alignItems="center">
              <Grid size={{ xs: 12, md: 3 }}>
                <TextField
                  label="Quotation No"
                  value={displayQuotationNumber || getValues('quotationNumber') || ''}
                  variant="outlined"
                  fullWidth
                  size="medium"
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <CustomerAutocomplete control={control} errors={errors} customersData={customersData} size="medium" />
              </Grid>
              <Grid size={{ xs: 12, md: 3 }}>
                <Controller
                  name="quotationDate"
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      inputProps={{ min: getValues('quotationDate') }}
                      InputLabelProps={{ shrink: true }}
                    />
                  )}
                />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <FormControl fullWidth variant="outlined" error={!!errors.employee}>
                      ...
                      <Select {...field} label="Employee" size="medium" value={field.value || ''}>
                        <MenuItem value="">
                          <em>None</em>
                        </MenuItem>
      ...
      <Drawer
        ...
        PaperProps={{ sx: { width: { xs: '100%', sm: 400 }, px: 6, py: 5 } }}
      >
        ...
        <Box sx={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 9, mt: 5 }}>
          <Box className="flex flex-col gap-3">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <TextField {...field} fullWidth size="medium" label="Reference No" error={!!errors.referenceNo} />
              )}
            />
          </Box>
          <Box className="flex flex-col gap-3">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <TextField {...field} fullWidth multiline minRows={3} size="medium" label="Notes" error={!!errors.notes} />
              )}
            />
            <Controller
            ...
            />
            <Button fullWidth variant="text" color="primary" startIcon={<Icon icon="mdi:file-document-outline" width={22} />} onClick={handleOpenTermsDialog}>
              Open Terms Editor
            </Button>
          </Box>
        </Box>
      ...
      <Grid size={{ xs: 12 }}>
        <Box sx={{ position: 'relative', mb: '280px' }}>
          ...
          <TotalsTwo
            layout="floating"
            control={control}
            primaryActionLabel={isEdit ? 'Update' : 'Complete'}
            onPrimaryAction={handleSubmit(handleFormSubmit, handleError)}
            secondaryActionLabel={!isEdit ? 'Save as Draft' : undefined}
            onSecondaryAction={!isEdit ? handleSubmit(handleDraftSubmit, handleError) : undefined}
          />
        </Box>
      </Grid>
      ...
      <Dialog open={termsDialogOpen} onClose={handleCloseTermsDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ py: 5, px: 5 }}>
          <TextField
            value={tempTerms || ''}
            onChange={event => setTempTerms(event.target.value)}
            fullWidth
            multiline
            rows={5}
            variant="filled"
            ...
            InputProps={{
              endAdornment: String(tempTerms || '').trim() !== '' && (
                <InputAdornment position="end">
                  <IconButton onClick={handleCloseTermsDialog} edge="end" title="Clear terms and conditions" size="small">
                    <Clear />
                  </IconButton>
                </InputAdornment>
              ),
