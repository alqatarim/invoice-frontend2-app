export function useInvoiceFormSubmission({ trigger, closeSnackbar, enqueueSnackbar, onSave, getValues }) {
  // Form submit handler
  const handleFormSubmit = async (data, errors, handleError) => {
    try {
      ...
      if (currentFormData) {
        ...
        const loadingKey = enqueueSnackbar(`Processing invoice...`, {
          variant: 'info',
          persist: true,
        ...
        if (!result.success) {
          const errorMessage = result.error?.message || result.message || `Failed to Processing invoice`;
