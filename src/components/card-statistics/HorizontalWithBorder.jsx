...
const HorizontalWithBorder = props => {
  ...
  const {
    ...
    fontSize,
    isCurrency,
    currencyIconWidth = '1rem',
    trendIcon,
    trendColor
  } = props
  const theme = useTheme()
  const resolvedTrendColor = trendColor && theme.palette?.[trendColor]?.main
    ? theme.palette[trendColor].main
    : theme.palette.text.primary
  const hasSubtitle = Boolean(subtitle)
  const hasTrend = trendIcon || (trendNumber !== undefined && trendNumber !== null)
        ...
        <div className='flex items-center gap-4 '>
          ...
          {isCurrency ? (
            <div className="flex items-center gap-0.5 min-w-[48px] justify-center">
              <Icon icon="lucide:saudi-riyal" color={theme.palette.text.primary} width={currencyIconWidth} />
              <Typography
                color="text.primary"
